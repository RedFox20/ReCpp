...
struct path
{
	...
	static int list_dirs(std::vector<std::string>& out, const std::string& directory, const char* matchPattern = "*")
	...
	static int list_files(std::vector<std::string>& out, const std::string& directory, const char* matchPattern = "*.*")
	...
	/** @return The current working directory of the application */
	static std::wstring working_dirw();


	/** @brief Set the working directory of the application to a new value */
	static void working_dir(const char* newDir);
	/** @brief Set the working directory of the application to a new value */
	static void working_dir(const wchar_t* newDir);
	template<class C> static void working_dir(cstring<C> newDir) { working_dir(newDir.c_str()); }
	template<class C> static void working_dir(cfpaths<C> newDir) { working_dir(newDir.str); }


	/** @brief Transform a relative path to a full path name */
	static std::string full_path(const char* relPath);
	/** @brief Transform a relative path to a full path name */
	static std::wstring full_path(const wchar_t* relPath);
	template<class C> static cstring<C> full_path(cstring<C> relPath) { return full_path(relPath.c_str()); }
	template<class C> static cstring<C> full_path(cfpaths<C> relPath) { return full_path(relPath.str); }


	/** @brief Extract the filename from a path name */
	static std::string file_name(const char* file);
	/** @brief Extract the filename from a path name */
	static std::wstring file_name(const wchar_t* file);
	template<class C> static cstring<C> file_name(cstring<C> file) { return file_name(file.c_str()); }
	template<class C> static cstring<C> file_name(cfpaths<C> file) { return file_name(file.str); }


	/** @brief Extract the foldername from a path name */
	static std::string folder_name(const char* path);
	/** @brief Extract the foldername from a path name */
	static std::wstring folder_name(const wchar_t* path);
	template<class C> static cstring<C> folder_name(cstring<C> path) { return folder_name(path.c_str()); }
	template<class C> static cstring<C> folder_name(cfpaths<C> path) { return folder_name(path.str); }
};
