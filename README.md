...
| **Ubuntu Linux** | clang++ 18–21, g++ 13–15 | x86_64, ASan/TSan/clang-tidy CI |
...
| `rpp.threading` | mutex, condition_variable, semaphore, concurrent_queue, thread_pool, threads, task, future_types, atomic_shared_ptr, close_sync, async |
...
|--------|--------|-------------|
| [Config](#rppconfigh) | [`config.h`](src/rpp/config.h) | Platform detection and compiler macros, with the base type aliases in [`config.types.h`](src/rpp/config.types.h) |
...
| [Async](#rppasynch) | [`async.h`](src/rpp/async.h) | Composable futures which own their shared state and never include `<future>` |
...
| Macro | Description |
|-------|-------------|
| [`RPP_SHORT_SIZE`](src/rpp/config.types.h#L10) | Size of `short` in bytes (platform-dependent) |
...
| Item | Description |
|------|-------------|
| [`future<T>`](src/rpp/async.h#L374) | Composable future which owns its shared state and names no std future |
| [`promise<T>`](src/rpp/async.h#L187) | Producer half of `future<T>`, which publishes one value or one exception |
| [`IsEventLoop`](src/rpp/async.h#L31) | Matches a loop which runs a posted callback on its own thread, as `rpp::event_loop` does |
| [`async(Task task)`](src/rpp/async.h#L316) | Runs a task on the thread pool, and returns `future<T>` |
| [`ready_future(T value)`](src/rpp/async.h#L655) | Creates a future which already holds the value |
| [`ready_future()`](src/rpp/async.h#L664) | Creates a `future<void>` which already finished |
| [`exceptional_future(std::exception_ptr e)`](src/rpp/async.h#L674) | Creates a future whose `get()` rethrows the exception `e` points at |
| [`exceptional_future(E e)`](src/rpp/async.h#L684) | Creates a future whose `get()` throws the exception |
| [`wait_all(futures)`](src/rpp/async.h#L691) | Blocks until every future holds its result |
| [`get_all(futures)`](src/rpp/async.h#L715) | Blocks and gathers every result. It collects every future, then rethrows the first exception |

### future Methods

| Method | Description |
|--------|-------------|
| [`~future()`](src/rpp/async.h#L395) | Collects a ready result, fails an assertion on a stored exception, and terminates for an unready future |
| [`operator=(future&& f)`](src/rpp/async.h#L383) | Ends the old state as the destructor does, then takes the state of `f` |
| [`valid()`](src/rpp/async.h#L398) | Returns `true` while the future holds a state, which `get()` consumes |
| [`get()`](src/rpp/async.h#L416) | Blocks, then returns the result or rethrows its exception. The future is invalid afterwards |
| [`wait()`](src/rpp/async.h#L409) | Blocks until the result arrives |
| [`wait_for(Duration timeout)`](src/rpp/async.h#L407) | Returns `wait_result::finished` when the result arrives before the timeout |
| [`wait_until(TimePoint until)`](src/rpp/async.h#L413) | Returns `wait_result::finished` when the result arrives before the deadline |
| [`then(Task task, Handlers... handlers)`](src/rpp/async.h#L453) | Continues with the result on the pool thread which published it, and the first handler whose exception type matches recovers the chain |
| [`then(Loop& loop, Task task, Handlers... handlers)`](src/rpp/async.h#L460) | Continues with the result on the thread of `loop`, as the other `then()` does on the pool |
| [`then(future<U>&& next)`](src/rpp/async.h#L467) | Follows this future with `next` and returns its result, and no thread waits for either one |
| [`then()`](src/rpp/async.h#L478) | Downcasts to `future<void>`, which drops the value |
| [`continue_with(Task task, Handlers... handlers)`](src/rpp/async.h#L486) | Runs the task with the result as `then()` does, and returns no future |
| [`continue_with(Loop& loop, Task task, Handlers... handlers)`](src/rpp/async.h#L494) | Runs the task with the result on the thread of `loop`, and returns no future |
| [`detach()`](src/rpp/async.h#L500) | Abandons the result without a wait. Nobody sees its exception |
| [`chain_async(Task task)`](src/rpp/async.h#L517) | Runs the task after this future. An invalid future starts it at once |
| [`chain_async(future&& next)`](src/rpp/async.h#L529) | Chains another future after this one |
| [`await_ready()`](src/rpp/async.h#L401) | Returns `true` when the result arrived, without a block |
| [`collect_ready(T* result)`](src/rpp/async.h#L425) | Collects a finished result without a block. Returns `false` while the result has not arrived |
| [`collect_wait(T* result)`](src/rpp/async.h#L428) | Blocks for the result and collects it. Returns `false` on an invalid future |
| [`await_suspend(coro_handle<> cont)`](src/rpp/async.h#L537) | Resumes the coroutine on the pool thread which publishes the result, as `then()` runs a task |
| [`await_resume()`](src/rpp/async.h#L547) | Returns the result to the coroutine, or rethrows its exception |
| [`promise_type`](src/rpp/async.h#L379) | Coroutine promise. It publishes the result after the frame destroys its locals, and before its by-value parameters |

### promise Methods

| Method | Description |
|--------|-------------|
| [`get_future()`](src/rpp/async.h#L218) | Returns the one future which receives the result, also after the promise published it |
| [`set_value(Args&&... args)`](src/rpp/async.h#L228) | Stores the value and wakes the waiter |
| [`set_exception(std::exception_ptr e)`](src/rpp/async.h#L235) | Stores the exception which `get()` rethrows, and wakes the waiter |
| [`~promise()`](src/rpp/async.h#L215) | Publishes a `std::logic_error` when the promise never published a result |
| [`operator=(promise&& p)`](src/rpp/async.h#L201) | Abandons the old state as the destructor does, then takes the state of `p` |
...
| [`make_ready_future(value)`](src/rpp/future.h#L999) | Create an already-completed future |
| [`make_exceptional_future(e)`](src/rpp/future.h#L1016) | Create an already-errored future |
| [`wait_all(const std::vector<cfuture<T>>& vf)`](src/rpp/future.h#L1025) | Block until all futures complete |
| [`get_all(futures)`](src/rpp/future.h#L1038) | Block and gather results from all futures |
...
| [`then(cfuture<U>&& next)`](src/rpp/future.h#L314) | Chain by waiting for this future, then returning the result of `next` |
| [`continue_with(Task task)`](src/rpp/future.h#L330) | Fire-and-forget continuation (moves `*this` into background) |
| [`continue_with(Task task, ExceptHA a, ...)`](src/rpp/future.h#L338) | Fire-and-forget continuation with 1–4 typed exception handlers |
| [`detach()`](src/rpp/future.h#L393) | Abandon future, wait in background (swallows exceptions) |
| [`chain_async(Task task)`](src/rpp/future.h#L425) | Sequential chaining: if invalid, starts a new async task; if valid, appends as continuation (swallows prior exceptions) |
| [`chain_async(cfuture&& next)`](src/rpp/future.h#L438) | Sequential chaining with another future |
| [`await_ready()`](src/rpp/future.h#L452) | Non-blocking check if the future is already finished |
| [`collect_ready(T* result)`](src/rpp/future.h#L483) | If already finished, collects the result into `*result` (non-blocking). Returns `true` if collected |
| [`collect_wait(T* result)`](src/rpp/future.h#L501) | If valid, blocks until finished and collects the result into `*result`. Returns `true` if collected |
| [`await_suspend(coro_handle<>)`](src/rpp/future.h#L512) | C++20 coroutine suspension point — waits on background thread, then resumes |
| [`await_resume()`](src/rpp/future.h#L531) | C++20 coroutine resume — returns the result, rethrows exceptions |
| [`promise_type`](src/rpp/future.h#L551) | C++20 coroutine promise enabling `rpp::cfuture<T>` as a coroutine return type |
...
Single-threaded coroutine tasks. Unlike `rpp::cfuture<T>` — whose awaiter spawns a detached thread-pool task on every `co_await` and resumes the continuation on a background worker — these never spawn a thread of their own and resume the awaiter on whatever thread the body completes on. Chained over event-loop leaves (`event_loop::delay` / `run_async`), a chain stays on the loop thread end-to-end. Two flavours share one implementation via a compile-time policy (zero runtime cost, identical API):
...
| Function | Description |
|----------|-------------|
| [`parallel_for(rangeStart, rangeEnd, maxRangeSize, func)`](src/rpp/thread_pool.h#L587) | Parallel for on the global thread pool |
| [`parallel_foreach(items, forEach)`](src/rpp/thread_pool.h#L608) | Parallel foreach on the global pool |
| [`parallel_task(task)`](src/rpp/thread_pool.h#L627) | Run async task on the global pool |
...
| Class | Description |
|-------|-------------|
| [`member_serialize<T>`](src/rpp/binary_serializer.h#L18) | Per-member serialization metadata |
| [`serializable<T>`](src/rpp/binary_serializer.h#L32) | CRTP base class — inherit and call `bind()` to auto-serialize |

### serializable Methods

| Method | Description |
|--------|-------------|
| [`bind(first, args, ...)`](src/rpp/binary_serializer.h#L87) | Bind struct members for serialization |
| [`bind_name(name, elem)`](src/rpp/binary_serializer.h#L81) | Bind with explicit name |
| [`serialize(binary_stream& out)`](src/rpp/binary_serializer.h#L95) | Serialize to binary stream |
| [`deserialize(binary_stream& in)`](src/rpp/binary_serializer.h#L103) | Deserialize from binary stream |
| [`serialize(string_buffer& out)`](src/rpp/binary_serializer.h#L113) | Serialize to string |
| [`deserialize(strview& in)`](src/rpp/binary_serializer.h#L127) | Deserialize from string |
...
| Type | Description |
|------|-------------|
| [`AtomicDuration`](src/rpp/atomic_timepoint.h#L24) | Lock-free atomic Duration with atomic arithmetic |
| [`AtomicTimePoint`](src/rpp/atomic_timepoint.h#L89) | Lock-free atomic TimePoint with atomic arithmetic |
| [`AtomicTimeSource`](src/rpp/atomic_timepoint.h#L135) | Lock-free time source with sync and warp offsets |
...
| Method | Description |
|--------|-------------|
| [`operator+=(Duration d)`](src/rpp/atomic_timepoint.h#L41) | Atomically add a Duration, returns new value |
| [`operator-=(Duration d)`](src/rpp/atomic_timepoint.h#L52) | Atomically subtract a Duration, returns new value |
| [`fetch_add(Duration d, memory_order)`](src/rpp/atomic_timepoint.h#L63) | Atomically add, returns old value |
| [`fetch_sub(Duration d, memory_order)`](src/rpp/atomic_timepoint.h#L72) | Atomically subtract, returns old value |
...
| Method | Description |
|--------|-------------|
| [`operator+=(Duration d)`](src/rpp/atomic_timepoint.h#L106) | Atomically add a Duration, returns new TimePoint |
| [`operator-=(Duration d)`](src/rpp/atomic_timepoint.h#L117) | Atomically subtract a Duration, returns new TimePoint |
...
| Type | Description |
|------|-------------|
| [`Timer`](src/rpp/timer.h#L19) | High-accuracy timer for profiling or deltaTime |
| [`StopWatch`](src/rpp/timer.h#L106) | Start/stop/resume event timer |
| [`ScopedPerfTimer`](src/rpp/timer.h#L172) | Auto-logs elapsed time from ctor to dtor |

### Timer Methods

| Method | Description |
|--------|-------------|
| [`Timer()`](src/rpp/timer.h#L35) | Construct and auto-start with default (Realtime) clock |
| [`Timer(ClockType clock)`](src/rpp/timer.h#L38) | Construct and auto-start with a specific clock type |
| [`Timer(ClockType clock, StartMode mode)`](src/rpp/timer.h#L44) | Construct with specific clock and start mode |
| [`start()`](src/rpp/timer.h#L53) | Start / restart the timer |
| [`time_now()`](src/rpp/timer.h#L50) | Get current time using this timer's clock type |
| [`elapsed()`](src/rpp/timer.h#L59) | Fractional seconds since start |
| [`elapsed_millis()`](src/rpp/timer.h#L61) | Fractional milliseconds since start |
| [`next()`](src/rpp/timer.h#L66) | Get elapsed time and restart |
| [`measure(Func&& func)`](src/rpp/timer.h#L85) | Measure a block's execution time (seconds) |
| [`measure_millis(Func&& func)`](src/rpp/timer.h#L93) | Measure a block's execution time (ms) |
...
| [`push(lock_t& lock_guard, T&& item)`](src/rpp/concurrent_queue.h#L343) | Push an item under a lock the caller holds, so no consumer sees it before the caller releases the lock |
| [`try_pop(T& out)`](src/rpp/concurrent_queue.h#L1029) | Non-blocking pop attempt |
| [`try_pop_all(std::vector<T>& out)`](src/rpp/concurrent_queue.h#L295) | Pop all items at once |
| [`wait_pop(Duration timeout)`](src/rpp/concurrent_queue.h#L549) | Blocking pop with timeout |
| [`wait_pop(T& outItem, Duration timeout)`](src/rpp/concurrent_queue.h#L642) | Blocking pop with predicate and timeout |
...
| [`await(Duration timeout)`](src/rpp/concurrent_queue.h#L1010) | C++20 coroutine `co_await` — wait for items available |
| [`await_pop(T& out, Duration timeout)`](src/rpp/concurrent_queue.h#L1041) | C++20 coroutine `co_await` — pop item (returns bool) |
| [`await_pop(Duration timeout)`](src/rpp/concurrent_queue.h#L1075) | C++20 coroutine `co_await` — pop item (returns `optional<T>`) |
...
| Class | Description |
|-------|-------------|
| [`load_balancer`](src/rpp/load_balancer.h#L13) | Rate-limiter for byte sending |

### Methods

| Method | Description |
|--------|-------------|
| [`can_send()`](src/rpp/load_balancer.h#L39) | True if send budget is available |
| [`wait_to_send(int bytes)`](src/rpp/load_balancer.h#L46) | Block until bytes can be sent |
| [`notify_sent(TimePoint now, int bytesToSend)`](src/rpp/load_balancer.h#L51) | Report bytes sent |
| [`get_max_bytes_per_sec()`](src/rpp/load_balancer.h#L27) | Get rate limit |
| [`set_max_bytes_per_sec(int n)`](src/rpp/load_balancer.h#L30) | Set rate limit |
...
| [`make_obfuscated(const char (&str)[N])`](src/rpp/obfuscated_string.h#L63) | Creates a compile-time obfuscated string from a string literal |
| [`operator ""_obfuscated`](src/rpp/obfuscated_string.h#L68) | String literal operator, in `rpp::literals` |
...
| Trait | Description |
|-------|-------------|
| [`function_traits<T>`](src/rpp/traits.h#L13) | Extracts `ret_type` and `arg_types` from functions, lambdas, member functions |
| [`first_arg_type<T>`](src/rpp/traits.h#L43) | First argument type of a callable |
