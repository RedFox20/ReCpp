...
| [`get_time_source_frame()`](src/rpp/event_loop.h#L290) | Snapshot the loop clock on the thread which builds a deadline |
...
Lock-free time source for simulation time warping and time synchronization with external sources. Maintains a `combined_offset` (sync + warp) applied to a base clock, Realtime by default. All mutations are atomic via `fetch_add`/`exchange`; reads use `memory_order_relaxed` for minimal overhead on the hot path (`time_now()`).

| Method | Description |
|--------|-------------|
| [`time_now()`](src/rpp/atomic_timepoint.h#L155) | Returns the base clock time plus the combined offset (sync + warp) |
| [`total_offset()`](src/rpp/atomic_timepoint.h#L177) | Returns the combined sync + warp offset |
| [`warp_offset()`](src/rpp/atomic_timepoint.h#L187) | Returns the current warp offset (diagnostic) |
| [`sync_offset()`](src/rpp/atomic_timepoint.h#L197) | Returns the current sync offset (diagnostic) |
| [`base_time_now()`](src/rpp/atomic_timepoint.h#L203) | Returns the base clock time without the sync and warp offsets |
| [`get_base_clock()`](src/rpp/atomic_timepoint.h#L206) | Returns the clock under the offsets, Realtime by default |
| [`set_base_clock(rpp::ClockType clock)`](src/rpp/atomic_timepoint.h#L212) | Sets the clock under the offsets. Monotonic keeps waits and timers through a wall clock step |
| [`warp_forward(Duration delta)`](src/rpp/atomic_timepoint.h#L217) | Atomically advances time by delta |
| [`warp_backward(Duration delta)`](src/rpp/atomic_timepoint.h#L226) | Atomically rewinds time by delta |
| [`set_sync_offset(Duration new_offset)`](src/rpp/atomic_timepoint.h#L235) | Sets sync offset, adjusting combined offset by the difference |
