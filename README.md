...
| Item | Description |
|------|-------------|
| [`future<T>`](src/rpp/async.h#L463) | Composable future which owns its shared state and names no std future |
| [`promise<T>`](src/rpp/async.h#L207) | Producer half of `future<T>`, which publishes one value or one exception |
...
| [`async(Task task)`](src/rpp/async.h#L408) | Runs a task on the thread pool, and returns `future<T>` |
| [`ready_future(T value)`](src/rpp/async.h#L766) | Creates a future which already holds the value |
| [`ready_future()`](src/rpp/async.h#L775) | Creates a `future<void>` which already finished |
| [`exceptional_future(std::exception_ptr e)`](src/rpp/async.h#L785) | Creates a future whose `get()` rethrows the exception `e` points at |
| [`exceptional_future(E e)`](src/rpp/async.h#L795) | Creates a future whose `get()` throws the exception |
| [`wait_all(futures)`](src/rpp/async.h#L802) | Blocks until every future holds its result |
| [`get_all(futures)`](src/rpp/async.h#L826) | Blocks and gathers every result. It collects every future, then rethrows the first exception |

### future Methods

| Method | Description |
|--------|-------------|
| [`~future()`](src/rpp/async.h#L489) | Collects a ready result, fails an assertion on a stored exception, and terminates for an unready future |
| [`operator=(future&& f)`](src/rpp/async.h#L477) | Ends the old state as the destructor does, then takes the state of `f` |
| [`valid()`](src/rpp/async.h#L492) | Returns `true` while the future holds a state, which `get()` consumes |
| [`get()`](src/rpp/async.h#L516) | Blocks, then returns the result or rethrows its exception. The future is invalid afterwards |
| [`wait()`](src/rpp/async.h#L498) | Blocks until the result arrives |
| [`wait_for(Duration timeout)`](src/rpp/async.h#L505) | Returns `wait_result::finished` when the result arrives before the timeout |
| [`wait_until(TimePoint until)`](src/rpp/async.h#L513) | Returns `wait_result::finished` when the result arrives before the deadline |
| [`then(Task task, Handlers... handlers)`](src/rpp/async.h#L554) | Continues with the result on the pool thread which published it, else on a new pool task. The first handler whose exception type matches recovers the chain |
| [`then(Loop& loop, Task task, Handlers... handlers)`](src/rpp/async.h#L561) | Continues with the result on the thread of `loop`, as the other `then()` does on the pool |
| [`then(future<U>&& next)`](src/rpp/async.h#L568) | Follows this future with `next` and returns its result, and no thread waits for either one |
| [`then()`](src/rpp/async.h#L579) | Downcasts to `future<void>`, which drops the value |
| [`continue_with(Task task, Handlers... handlers)`](src/rpp/async.h#L588) | Runs the task with the result as `then()` does, and returns no future. An error which no handler takes goes to `LogWarning()` |
| [`continue_with(Loop& loop, Task task, Handlers... handlers)`](src/rpp/async.h#L596) | Runs the task with the result on the thread of `loop`, and returns no future. An error which no handler takes goes to `LogWarning()` |
| [`detach()`](src/rpp/async.h#L602) | Abandons the result without a wait. Nobody sees its exception |
| [`chain_async(Task task)`](src/rpp/async.h#L619) | Runs the task after this future. An invalid future starts it at once |
| [`chain_async(future&& next)`](src/rpp/async.h#L627) | Chains another future after this one |
| [`await_ready()`](src/rpp/async.h#L495) | Returns `true` when the result arrived, without a block |
| [`collect_ready(T* result)`](src/rpp/async.h#L525) | Collects a finished result without a block. Returns `false` while the result has not arrived |
| [`collect_wait(T* result)`](src/rpp/async.h#L528) | Blocks for the result and collects it. Returns `false` on an invalid future |
| [`await_suspend(coro_handle<> cont)`](src/rpp/async.h#L635) | Resumes the coroutine on the pool thread which publishes the result, else on a new pool task, as `then()` runs a task |
| [`await_resume()`](src/rpp/async.h#L645) | Returns the result to the coroutine, or rethrows its exception |
| [`promise_type`](src/rpp/async.h#L473) | Coroutine promise. It publishes after the frame destroys its locals and before its parameters, so the next step starts as a pool task |

### promise Methods

| Method | Description |
|--------|-------------|
| [`get_future()`](src/rpp/async.h#L235) | Returns the one future which receives the result, also after the promise published it |
| [`set_value(Args&&... args)`](src/rpp/async.h#L245) | Stores the value and wakes the waiter. A second publisher gets a `std::logic_error`, as with `std::promise` |
| [`set_exception(std::exception_ptr e)`](src/rpp/async.h#L254) | Stores the exception which `get()` rethrows, and wakes the waiter. A second publisher gets a `std::logic_error` |
| [`~promise()`](src/rpp/async.h#L232) | Publishes a `std::logic_error` when the promise never published a result |
| [`operator=(promise&& p)`](src/rpp/async.h#L219) | Abandons the old state as the destructor does, then takes the state of `p` |
...
| [`await_resume()`](src/rpp/future.h#L549) | C++20 coroutine resume — returns the result, rethrows exceptions |
