...
// TODO: find a way to deduplicate test for queue with pthread mutex and condition_variable

TestImpl(test_concurrent_queue_pthread)
{
...
#endif
    #define CONCURRENT_QUEUE rpp::concurrent_queue<std::string, rpp::shm_mutex, rpp::shm_cond_var>
    using Clock = CONCURRENT_QUEUE::clock;
    using Millis = std::chrono::milliseconds;

    TestInit(test_concurrent_queue_pthread)
    {
    }
    ...
    PopResult wait_pop_timed(CONCURRENT_QUEUE& queue, Clock::duration timeout)
    {
        PopResult r;
        rpp::Timer t;
    ...
    PopResult wait_pop_until(CONCURRENT_QUEUE& queue, Clock::time_point until)
    {
        PopResult r;
        rpp::Timer t;
