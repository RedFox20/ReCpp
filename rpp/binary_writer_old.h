...
namespace rpp /* ReCpp */
{
     ...
     */
    template<class write_impl> struct binary_writer_old final : public writer_base, public write_impl
    {
        ...
        template<class T> binary_writer_old& write(const T& data) noexcept {
            write_impl::write(data);
            return *this;
        }
        /** @brief Appends data from other binary_writer to this one */
        template<class C> binary_writer_old& write(const binary_writer_old<C>& wb) noexcept {
            write(wb.data(), wb.size());
            return *this;
        }

        binary_writer_old& write_byte(byte value)     noexcept { return write(value); } /** @brief Writes a 8-bit unsigned byte into the buffer */
        binary_writer_old& write_short(short value)   noexcept { return write(value); } /** @brief Writes a 16-bit signed short into the buffer */
        binary_writer_old& write_ushort(ushort value) noexcept { return write(value); } /** @brief Writes a 16-bit unsigned short into the buffer */
        binary_writer_old& write_int(int value)       noexcept { return write(value); } /** @brief Writes a 32-bit signed integer into the buffer */
        binary_writer_old& write_uint(uint value)     noexcept { return write(value); } /** @brief Writes a 32-bit unsigned integer into the buffer */
        binary_writer_old& write_int64(int64 value)   noexcept { return write(value); } /** @brief Writes a 64-bit signed integer into the buffer */
        binary_writer_old& write_uint64(uint64 value) noexcept { return write(value); } /** @brief Writes a 64-bit unsigned integer into the buffer */

        /** @brief Write a length specified string to the buffer in the form of [uint16 len][data] */
        template<class Char> binary_writer_old& write_nstr(const Char* str, int len) noexcept {
            write_int(len).write(str, len * sizeof(Char)); return *this;
        }
        binary_writer_old& write(const strview& str)      noexcept { return write_nstr(str.str, str.len); }
        binary_writer_old& write(const std::string& str)  noexcept { return write_nstr(str.c_str(), (int)str.length()); }
        binary_writer_old& write(const std::wstring& str) noexcept { return write_nstr(str.c_str(), (int)str.length()); }
         ...
         */
        template<class T, class U> binary_writer_old& write(const std::vector<T, U>& vec) noexcept {
    ...
    template<class C> binary_writer_old<C>& operator<<(binary_writer_old<C>& w, const strview& v)      noexcept { return w.write(v); }
    template<class C> binary_writer_old<C>& operator<<(binary_writer_old<C>& w, const std::string& v)  noexcept { return w.write(v); }
    template<class C> binary_writer_old<C>& operator<<(binary_writer_old<C>& w, const std::wstring& v) noexcept { return w.write(v); }
    template<class C> binary_writer_old<C>& operator<<(binary_writer_old<C>& w, bool v)   noexcept { return w.write(v); }
    template<class C> binary_writer_old<C>& operator<<(binary_writer_old<C>& w, char v)   noexcept { return w.write(v); }
    template<class C> binary_writer_old<C>& operator<<(binary_writer_old<C>& w, byte v)   noexcept { return w.write(v); }
    template<class C> binary_writer_old<C>& operator<<(binary_writer_old<C>& w, short v)  noexcept { return w.write(v); }
    template<class C> binary_writer_old<C>& operator<<(binary_writer_old<C>& w, ushort v) noexcept { return w.write(v); }
    template<class C> binary_writer_old<C>& operator<<(binary_writer_old<C>& w, int v)    noexcept { return w.write(v); }
    template<class C> binary_writer_old<C>& operator<<(binary_writer_old<C>& w, uint v)   noexcept { return w.write(v); }
    template<class C> binary_writer_old<C>& operator<<(binary_writer_old<C>& w, int64 v)  noexcept { return w.write(v); }
    template<class C> binary_writer_old<C>& operator<<(binary_writer_old<C>& w, uint64 v) noexcept { return w.write(v); }
    template<class C> binary_writer_old<C>& operator<<(binary_writer_old<C>& w, float v)  noexcept { return w.write(v); }
    template<class C> binary_writer_old<C>& operator<<(binary_writer_old<C>& w, double v) noexcept { return w.write(v); }
    template<class C, class T, class U> binary_writer_old<C>& operator<<(binary_writer_old<C>& w, const std::vector<T, U>& v) noexcept { return w.write(v); }
    template<class C> binary_writer_old<C>& operator<<(binary_writer_old<C>& w, binary_writer_old<C>& m(binary_writer_old<C>&)) noexcept { return m(w); }
    template<class C> binary_writer_old<C>& endl(binary_writer_old<C>& w) noexcept { w.flush(); return w; }
    ...
    template<int SIZE = 512> using array_writer = binary_writer_old<array_write<SIZE>>;
    /** @brief Writes data into an array view. Array size depends on its initialized view size. */
    using view_writer   = binary_writer_old<view_write>;
    /** @brief Writes data into a dynamically growing buffer. Buffer grows aligned to 512 bytes. */
    using buffer_writer = binary_writer_old<buffer_write>;
    /** @brief Writes data to file using C FILE* API. */
    using file_writer   = binary_writer_old<file_write>;
    /** @brief Writes data directly to an rpp::socket */
    using socket_writer = binary_writer_old<socket_write>;
    /**
     ...
     */
    template<class buffer, class storage> using stream_writer = binary_writer_old<composite_write<buffer, storage>>;
