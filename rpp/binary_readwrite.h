...
namespace rpp /* ReCpp */ 
{
	...
	struct socket_writer
	{
		...
		socket_writer& write(const void* data, uint numBytes);
		/** @brief Writes object of type T into the buffer */
		template<class T> socket_writer& write(const T& data) {
		...
		socket_writer& write_short(short value)   { return write(value); } /** @brief Writes a 16-bit signed short into the buffer */
		socket_writer& write_ushort(ushort value) { return write(value); } /** @brief Writes a 16-bit unsigned short into the buffer */
		socket_writer& write_int(int value)       { return write(value); } /** @brief Writes a 32-bit signed integer into the buffer */
		socket_writer& write_uint(uint value)     { return write(value); } /** @brief Writes a 32-bit unsigned integer into the buffer */
		socket_writer& write_int64(int64 value)   { return write(value); } /** @brief Writes a 64-bit signed integer into the buffer */
		socket_writer& write_uint64(uint64 value) { return write(value); } /** @brief Writes a 64-bit unsigned integer into the buffer */
		...
		socket_writer& write(const std::wstring& str) { return write_nstr(str.c_str(), (int)str.length()); }
	};
	// explicit operator<< overloads
	inline socket_writer& operator<<(socket_writer& w, const strview& v)      { return w.write(v); }
	inline socket_writer& operator<<(socket_writer& w, const std::string& v)  { return w.write(v); }
	inline socket_writer& operator<<(socket_writer& w, const std::wstring& v) { return w.write(v); }
	inline socket_writer& operator<<(socket_writer& w, bool v)   { return w.write(v); }
	inline socket_writer& operator<<(socket_writer& w, char v)   { return w.write(v); }
	inline socket_writer& operator<<(socket_writer& w, byte v)   { return w.write(v); }
	inline socket_writer& operator<<(socket_writer& w, short v)  { return w.write(v); }
	inline socket_writer& operator<<(socket_writer& w, ushort v) { return w.write(v); }
	inline socket_writer& operator<<(socket_writer& w, int v)    { return w.write(v); }
	inline socket_writer& operator<<(socket_writer& w, uint v)   { return w.write(v); }
	inline socket_writer& operator<<(socket_writer& w, int64 v)  { return w.write(v); }
	inline socket_writer& operator<<(socket_writer& w, uint64 v) { return w.write(v); }
	inline socket_writer& operator<<(socket_writer& w, float v)  { return w.write(v); }
	inline socket_writer& operator<<(socket_writer& w, double v) { return w.write(v); }
	inline socket_writer& operator<<(socket_writer& w, socket_writer& m(socket_writer&)) { return m(w); }
	inline socket_writer& endl(socket_writer& w) { w.flush(); return w; }
	...
	struct socket_reader
	{
		...
		template<class Char> socket_reader& read(std::basic_string<Char>& str) {
		...
		template<class Char> socket_reader& peek(std::basic_string<Char>& str) {
			uint n = min<uint>(read_ushort(), available());
			str.resize(n);
	...
	// explicit operator>> overloads
	inline socket_reader& operator>>(socket_reader& r, strview& v)      { r.peek(v); return r; }
	inline socket_reader& operator>>(socket_reader& r, std::string& v)  { r.read(v); return r; }
	inline socket_reader& operator>>(socket_reader& r, std::wstring& v) { r.read(v); return r; }
	inline socket_reader& operator>>(socket_reader& r, bool& v)   { r.read(v); return r; }
	inline socket_reader& operator>>(socket_reader& r, char& v)   { r.read(v); return r; }
	inline socket_reader& operator>>(socket_reader& r, byte& v)   { r.read(v); return r; }
	inline socket_reader& operator>>(socket_reader& r, short& v)  { r.read(v); return r; }
	inline socket_reader& operator>>(socket_reader& r, ushort& v) { r.read(v); return r; }
	inline socket_reader& operator>>(socket_reader& r, int& v)    { r.read(v); return r; }
	inline socket_reader& operator>>(socket_reader& r, uint& v)   { r.read(v); return r; }
	inline socket_reader& operator>>(socket_reader& r, int64& v)  { r.read(v); return r; }
	inline socket_reader& operator>>(socket_reader& r, uint64& v) { r.read(v); return r; }
	inline socket_reader& operator>>(socket_reader& r, float& v)  { r.read(v); return r; }
	inline socket_reader& operator>>(socket_reader& r, double& v) { r.read(v); return r; }
	inline socket_reader& operator>>(socket_reader& r, socket_reader& m(socket_reader&)) { return m(r); }
